...
// JSpec - Core - Copyright TJ Holowaychuk <tj@vision-media.ca> (MIT Licensed) 2008-2012
//
//

Promise.each = function(arr, fn) {
  return arr.reduce(function(prev, cur) { 
    return prev.then(function () { return fn(cur) })
  }, Promise.resolve());
}
...
;(function(){
  ...
  JSpec = {
    ...
    evalBody : function(body, errorMessage, done) {
      ...
      with (dsl){ with (context) { with (matchers) { eval(JSpec.awaitTransform(contents)) }}}
      if (done && !/\bdone[()]|^\s*await\s/m.test(contents)) done();
      delete context.done;
    },
    ...
    run : function(options) {
      ...
      return Promise.each(this.suites, function(suite) {
         return new Promise(function (resolve) {
           JSpec.runSuite(suite).then(resolve);
         })
      })
    },
    ...
    runSuite : function(suite) {
      ...
      return new Promise(function(resolve) {
        if (!suite.isShared())
        {
          ...
          self.evalBody(suite.body)
          suite.ran = true
          hook('beforeSuite', suite), suite.hook('before'), suite.hook('before_nested')
          Promise.each(suite.specs, function(spec) {
            return new Promise(function(resolve) {
              hook('beforeSpec', spec)
              suite.hook('before_each')
              JSpec.runSpec(spec).then(function () {
                hook('afterSpec', spec)
            ...
            new Promise(function (resolve) {
              if (suite.hasSuites()) {
                Promise.each(suite.suites, function(suite) {
                  return JSpec.runSuite(suite)
                }).then(resolve)
              }
              else
                resolve()
    ...
    runSpec : function(spec) {
      ...
      return new Promise(function(resolve) {
        ...
        new Promise(function(resolve) {
          new Promise(function (resolve) {
            self.evalBody(spec.body, "", resolve)
          })
          .then(resolve)
          .catch(function (e) {
            fail(e)
          ...
          destub()
          self.stats.specsFinished++
          self.stats.assertions += spec.assertions.length
          resolve()
        })
      })
    },
    ...
    exec : function(file) {
      ...
      eval('with (JSpec){' + this.awaitTransform(this.preprocess(this.load(file))) + '}')
      return this
    },

    /**
     * Enable limited use of inline await within test spec.
     *   Note: JSpec does not currently support full async/await,
     *           but it can handle a simple inline await statement.
     *
     * @param {string} spec
     * @api private
     */

    awaitTransform : function(spec) {
      if (/^\s*await\s/m.test(spec))
        return spec.replace(/^\s*await\s+/gm, "/*await*/")
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        if (/[.]then[(]function/.test(line)) continue;

        if (inThen && /^\s*\}\);/.test(line)) {
          inThen = false;
          if (numAwait > 0)
            lines[i] = "done()" + lines[i]
          for (; numAwait > 0; numAwait--)
            lines[i] += "})"
        }

        if (/\*await\*.+\);?\s*$/.test(line)) {
          lines[i] += ".then(function() {"
          inThen = true;
          numAwait++;
        } else if (/\*await\*/.test(line)) {
          console.warn("JSpec does not support 'await' on lines that do not end with a ')'; please use 'new Promise(...).then(done) instead")
        }
      }
      return lines.join("\n")
    }
